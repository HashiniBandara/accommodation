...
( function( $ )
{
	...
	$.fn.sortableLists = function( options )
	{
		...
		// $('body').on('mousedown touchstart', '.s-l-opener', function(e){
		...
		function endDrag( e )
		{
			...
			cEl.el.animate( { left: offset.left - state.cEl.mL, top: offset.top - state.cEl.mT }, 250,
				function()  // complete callback
				{
					...
					{
						hintWrapperNode.parent().children( 'div' ).first().append( opener.clone( true ) );
					}
			...
			$( setting.listSelector, state.rootEl.el ).each( function( i )
				{
					...
					{
						$( this ).parent().find( 'div' ).eq(0).children( '.s-l-opener' ).first().remove();
						$( this ).remove();
					}
				}
			);
